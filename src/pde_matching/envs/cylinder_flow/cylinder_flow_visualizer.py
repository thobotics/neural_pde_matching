...
class CylinderFlowVisualizer(Visualizer):
    ...
    def visualize(
        ...
        if hasattr(data_batch, "batch") and data_batch.batch is not None:
            ...
            u_list = [
                u[batch_idx == i]
                for i in range(n_images if n_images != -1 else len(data_list))
            ]
        ...
        def plot_cylinder_flow(
            ax, data, u_state, properties, title_suffix, vel_min=None, vel_max=None
        ):
            ...
            if len(positions) > 0:
                ...
                ax.set_xlim(
                    positions[:, 0].min() - x_margin, positions[:, 0].max() + x_margin
                )
                ax.set_ylim(
                    positions[:, 1].min() - y_margin, positions[:, 1].max() + y_margin
                )
        ...
        if not self.overlayed_ground_truth:
            ...
            axes = [
                fig.add_subplot(num_images, 2, i * 2 + j + 1)
            ...
            for i, ax in enumerate(axes):
                ...
                ax.tricontour(
                    triang, vel_mag_gt, levels=10, colors="red", alpha=0.8, linewidths=1
                )
                ...
                if i == 0:
                    ...
                    legend_elements = [
                        Patch(facecolor="viridis", alpha=0.7, label="Prediction"),
                        Patch(
                            facecolor="red", alpha=0.8, label="Ground Truth (contours)"
                        ),
                    ]
                ...
                if len(positions) > 0:
                    ...
                    y_margin = (positions[:, 1].max() - positions[:, 1].min()) * 0.05
                    ax.set_xlim(
                        positions[:, 0].min() - x_margin,
                        positions[:, 0].max() + x_margin,
                    )
                    ax.set_ylim(
                        positions[:, 1].min() - y_margin,
                        positions[:, 1].max() + y_margin,
                    )
        ...
        title = (
            "Cylinder Flow: Prediction vs Ground Truth"
            if not self.overlayed_ground_truth
            else "Cylinder Flow: Prediction with Ground Truth Overlay"
        )
        fig.suptitle(title)
