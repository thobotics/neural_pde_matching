...
class CylinderFlowEnv(TrajectoryDatasetMixin, Env):
        ...
        if self._n_total_sequences is None:
            ...
            if stage_dir.exists():
                self._n_total_sequences = len(
                    [f for f in stage_dir.iterdir() if re.match(r"[0-9]+\.pt", f.name)]
                )
            else:
    ...
    def _extract_pos_from_state_or_graph(
        self, state: torch.Tensor, graph: Data
    ) -> torch.Tensor:
    ...
    def triangles_to_edges(self, faces):
        ...
        packed_edges = (
            senders.long() * 100000 + receivers.long()
        )  # Assuming max node < 100k
    ...
    def process(self):
        ...
        try:
            ...
            raise ImportError(
                "tfrecord_lite is required. Install with: pip install tfrecord-lite"
            )
    ...
    def _process_stage(
        self, tfrecord_path: Path, stage_dir: Path, meta: dict, stage: str
    ):
        ...
        for example_idx, example in enumerate(
            tqdm(tf_record_iterator(str(tfrecord_path)), desc=f"Processing {stage}")
        ):
            # Extract features and create trajectory data
            trajectory_data = self._extract_features(
                example, meta, is_first=(example_idx == 0)
            )
    ...
    def _extract_features(
        self, example: dict, meta: dict, is_first: bool = False
    ) -> List[Data]:
        ...
        def extract_feature_to_torch(name: str, expected_dtype: str):
            ...
            # Get raw bytes
            raw_data = (
                example[name][0] if isinstance(example[name], list) else example[name]
            )
            ...
            return torch.tensor(
                data, dtype=torch.float32 if dtype == np.float32 else torch.long
            )
        ...
        for i in range(edge_index.shape[1]):
            ...
            if src_type in [
                ...
                NodeType.INFLOW,
                NodeType.OUTFLOW,
            ] and dst_type in [NodeType.NORMAL, NodeType.INFLOW, NodeType.OUTFLOW]:
                edge_types[i] = EdgeType.NORMAL_NORMAL
            elif (
                src_type in [NodeType.NORMAL, NodeType.INFLOW, NodeType.OUTFLOW]
                and dst_type == NodeType.WALL_BOUNDARY
            ):
                edge_types[i] = EdgeType.NORMAL_BOUNDARY
            elif (
                dst_type in [NodeType.NORMAL, NodeType.INFLOW, NodeType.OUTFLOW]
                and src_type == NodeType.WALL_BOUNDARY
            ):
        ...
        for t_idx, t in enumerate(timestep_indices):
            ...
            u_state = torch.cat([velocity[t], pressure[t]], dim=-1)
        ...
        for t in range(len(trajectory_data)):
            if t == len(trajectory_data) - 1:
                ...
                trajectory_data[t].u_dot = (
                    trajectory_data[t + 1].u - trajectory_data[t].u
                ) / self.dt
        ...
        for t in range(len(trajectory_data)):
            if t == len(trajectory_data) - 1:
                ...
                trajectory_data[t].u_dot_dot = (
                    trajectory_data[t + 1].u_dot - trajectory_data[t].u_dot
                ) / self.dt
...
@Env.register("CylinderFlowLongEnv")
class CylinderFlowLongEnv(CylinderFlowEnv):
    @property
    def trajectory_length(self) -> int:
        return 102
